...
# Read in cabone model
  # cabone_mod <- mrgsolve::mread("model/cabone_perera_MA.cpp")  # cabone model
  cabone_mod <- mrgsolve::mread("model/cabone_perera_noMA.cpp")  # cabone model
...
# Source population
  source("scripts/190717_Population_cabone.R")
  
# Create simulation input dataset
# Define time points
  conc_times <- c(0, 0.5, 1, 1.5, 2, 4, 6, 8, 10, 24)
  dose_times <- 0  # 1 dose
  dose_amt <- 100
...
#   dosage events for mrgsolve
  concdose_tb <- tibble::tibble(
    ...
    rate = 0,
    SIM = rep(1:nsim, each = nobs*nid/nsim)
  )
...
# Save data as .RDS for later use
  ...
  output_tb %>% dplyr::select(
    ID, time, evid, amt, cmt, SIM, PKKA, PKK12, PKK21, 
    PKV1, PKV2, PKTLAG, PKCLCAPX, PKCLCAO, PKCLPX
