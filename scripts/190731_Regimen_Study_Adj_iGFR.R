...
# GFR debug
  output_tb %>% 
    dplyr::filter(cmt == 1 & time < 2) %>% 
    dplyr::select(ID, time, GFR, UCA, B, T10, T20, J27, CaFilt, CaReabsActive)
...
# Plasma Calcium Residuals vs. Time
  ...
  ggsave("output/PlasResVsTime_iGFR.png", width = 10.7, height = 8.03)
  
  p1_facet <- p1 + facet_wrap(~TRT)
  ggsave("output/PlasResVsTimeFacet_iGFR.png", width = 10.7, height = 8.03)
...
# Plasma Calcium Residuals vs. Predicted
  ...
  p2 <- p + scale_y_continuous("Residual (mmol)", limits = c(-5, 5), 
    breaks = c(-5, -3, -1, 0, 1, 3, 5))
  ggsave("output/PlasResVsPred_iGFR.png", width = 10.7, height = 8.03)
  
  p2_facet <- p2 + facet_wrap(~TRT)
  ggsave("output/PlasResVsPredFacet_iGFR.png", width = 10.7, height = 8.03)
...
# Urine Calcium Residuals vs. Treatment Arm
  ...
  ggsave("output/UrinResVsTrt_iGFR.png", width = 10.7, height = 8.03)
