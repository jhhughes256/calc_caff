...
// Read into R using mread function (?mrgsolve::mread for info)
...
$SET atol = 1e-8, rtol = 1e-8, maxsteps = 100000
...
F_DENSC = DENF*1E6;
F_TERISC = TERIF;
double PKKA = TVPKKA*exp(ETA1);
double PKK12 = TVPKK12*exp(ETA2);
double PKK21 = TVPKK21*exp(ETA3);
double PKV1 = TVPKV1*exp(ETA4);
double PKV2 = TVPKV2*exp(ETA5);
double PKTLAG = TVPKTLAG*exp(ETA6);
double PKCLCAPX = TVPKCLCAPX*exp(ETA7);
double PKCLCAO = TVPKCLCAO*exp(ETA8);
double PKCLPX = TVPKCLPX*exp(ETA9);
ALAG_PKGUT = PKTLAG;
...
[PARAM]
  ...
  // Caffeine/Paraxanthine PK parameters from...
  ...
  //  ##  Activity. 
  //  ## Drug Metab Disposition. 2013;41(11):1957-1966.
  ...
  // Allocated in population (idata_set) so set to zero
  ETA1 = 0  // PPVPKKA
  ETA2 = 0  // PPVPKK12
  ETA3 = 0  // PPVPKK21
  ETA4 = 0  // PPVPKV1
...
[ODE]
  ...
  //* Effect of caffeine/paraxanthine on reabsorption */
  double ReabsCAFeff = 1 - (PDEMAX*PKC1/(PKC1 + 5.8257));
  
  //* PTH-sensitive calcium reabsorption in kidney */
  //* Reabs50 = 1.573 = H(4-u)-delta */
  double CaReabsActive =  (ReabsMax*C1/(Reabs50 + C1))*ReabsPTHeff*ReabsCAFeff;
  ...
  //* Caffeine throught to impact calcium reabsorption via adenosine receptor
  ...
  double PKC1 = PKCENT/PKV1;
  double PKC2 = PKPARA/PKV2;

  dxdt_PKGUT = -PKKA*PKGUT;
  dxdt_PKCENT = PKF1*PKKA*PKGUT - PKCENT*PKK12 + PKPERI*PKK21 - 
    PKC1*PKCLCAPX - PKC1*PKCLCAO;
  dxdt_PKPERI = PKCENT*PKK12 - PKPERI*PKK21;
  dxdt_PKPARA = PKC1*PKCLCAPX - PKC2*PKCLPX;
  //* CAFFEINE/PARAXANTHINE  */
...
capture DVCA = PKC1*(1 + EPS1) + PKLLOQ;  //* caffeine observed concentration */
capture DVPX = PKC2*(1 + EPS2) + PKLLOQ;  //* paraxanthine observed concentration */

[CAPTURE]
DENMOL T43 DENCP UCA PKC1 PKC2 PKGUT PKCENT PKPERI PKPARA 
PKKA PKK12 PKK21 PKV1 PKV2 PKTLAG PKCLCAPX PKCLCAO PKCLPX
T10 T20 J27 CaFilt CaReabsActive
